...
// elements are chosen as pivots, so we'll ask you to explore three different pivoting rules.

// You should not count comparisons one-by-one. Rather, when there is a recursive call
// on a subarray of length m
// m, you should simply add m - 1 to your running total of comparisons. (This is because
// the pivot element is compared to each of the other m − 1 elements in the subarray in
// this recursive call.)
...
const firstElementQuickSort = arr => {};

// DIRECTIONS FOR THIS PROBLEM:
// Compute the number of comparisons (as in Problem 1), always using the final element of
// the given array as the pivot element. Again, be sure to implement the Partition subroutine
// exactly as it is described in the video lectures.

// Recall from the lectures that, just before the main Partition subroutine, you should exchange
// the pivot element (i.e., the last element) with the first element.

const finalElementQuickSort = arr => {};
...
// performance on input arrays that are nearly sorted or reverse sorted.] In more detail, you
// should choose the pivot as follows. Consider the first, middle, and final elements of the give
// array. (If the array has odd length it should be clear what the "middle" element is; for an
// array with even length 2k, use the kth element as the "middle" element. So for the array
...
// EXAMPLE: For the input array 8 2 4 5 7 1 you would consider the first (8), middle (4), and
// last (1) elements; since 4 is the median of the set {1,4,8}, you would use 4 as your pivot
// element.

// SUBTLE POINT: A careful analysis would keep track of the comparisons made in identifying the
// median of the three candidate elements. You should NOT do this. That is, as in the previous two
// problems, you should simply add m−1 to your running total of comparisons every time you recurse
// on a subarray with length m.

const medianOfThreeQuickSort = arr => {};
